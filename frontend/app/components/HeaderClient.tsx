  ...
  return (
    <header
      className={`fixed inset-0 z-50 h-24 flex items-center transition-colors duration-[1200ms] ease-in-out ${
        isSticky ? 'bg-[#ff5500]/95 shadow-lg' : 'bg-transparent'
      }`}
    >
      <div className="container py-6 px-2 sm:px-6">
        <div className="flex items-center justify-between gap-5">
          <Link className="flex items-center gap-2" href="/" aria-label={settingsTitle}>
            <span
              className={`${orbitron.className} select-none tracking-widest text-white text-xl sm:text-3xl font-black leading-none`}
            >
              TIMBERLINEUPFITTERS
            </span>
          </Link>
          ...
          <nav>
            ...
            >
              <li>
                <Link href="/about" className="hover:underline text-white/90 hover:text-white">
                  About
                </Link>
              </li>

              <li className="relative group">
                <button className="hover:underline flex items-center gap-1 text-white/90 hover:text-white">
                  Manufacturers
              ...
              <li className="sm:before:w-[1px] sm:before:bg-white/30 before:block flex sm:gap-4 md:gap-6">
                <Link
                  className="rounded-full flex gap-4 items-center bg-black/80 hover:bg-black py-2 px-4 justify-center sm:py-3 sm:px-6 text-white transition-colors duration-200"
                  href="https://github.com/sanity-io/sanity-template-nextjs-clean"
                  target="_blank"
                  rel="noopener noreferrer"
                >
                  <span className="whitespace-nowrap">View on GitHub</span>
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="hidden sm:block h-4 sm:h-6">
